...
def download_video(url, output_dir="./tiktok_analysis"):
    ...
    try:
        ...
        print(f"[OK] 视频已下载：{video_path}")
        return video_path
    except subprocess.CalledProcessError as e:
        print(f"[ERR] 下载失败：{e}")
        return None
    except FileNotFoundError:
        print("[ERR] yt-dlp未安装！请先安装：pip install yt-dlp")
        return None
...
def extract_frames(video_path, output_dir="./tiktok_analysis/frames", num_frames=5):
    ...
    try:
        ...
        print(f"[OK] 已提取 {len(frames)} 张关键帧")
        return frames
...
def extract_audio(video_path, output_dir="./tiktok_analysis"):
    ...
    try:
        subprocess.run(cmd, check=True, capture_output=True, stderr=subprocess.DEVNULL)
        print(f"[OK] 音频已提取：{audio_path}")
        return audio_path
    except Exception as e:
        print(f"[ERR] 音频提取失败：{e}")
        return None
    ...
    try:
        ...
        if os.path.exists(transcript_file):
            with open(transcript_file, "r", encoding="utf-8") as f:
                transcript = f.read()
            print(f"[OK] 转录完成：{len(transcript)} 字符")
            return transcript
        else:
            print("[警告] 转录文件未生成")
            return None
